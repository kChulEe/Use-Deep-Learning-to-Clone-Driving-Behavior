...
def telemetry(sid, data):
    if data:
        ...
        #resize image to match the model input image size
        image_array = resize(image_array, resize_shape=(64,64))
        transformed_image_array = image_array[None, :, :, :]
        steering_angle = float(model.predict(image_array[None, :, :, :], batch_size=1))

        #constant throttle for run 1
        throttle = 0.3

        ###########    for run 2, slow down the car
        if float(speed) < 6:
        	throttle = 10
